...
// ✅ Get all rejected events (for review or re-approval)
router.get('/events/rejected', auth('admin'), async (req, res) => {
  const rejectedEvents = await Event.find({ status: 'rejected' }).populate('organizer', 'name email');
